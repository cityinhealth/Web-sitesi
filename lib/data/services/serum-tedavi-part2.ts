// Additional serum services - Part 2
export const SERUM_SERVICES_PART2 = [
    {
        title: "Myers Kokteyli (Enerji Serumu) Tedavisi",
        slug: "myers-kokteyli-tedavisi",
        short_description: "B vitaminleri, C vitamini, magnezyum ve kalsiyum içeren klasik enerji ve bağışıklık serumu.",
        cover_image: "/images/myers-kokteyli.png",
        meta_title: "Evde Myers Kokteyli Tedavisi İstanbul | Enerji Serumu | City in Health",
        meta_description: "Myers Kokteyli enerji serum tedavisi İstanbul'da evinizde. Yorgunluk, migren, bağışıklık desteği. 39 ilçede 7/24.",
        content_html: `
<h2>Myers Kokteyli Nedir?</h2>
<p>Myers Kokteyli, Dr. John Myers tarafından geliştirilen ve dünya genelinde en çok uygulanan IV vitamin infüzyonudur. B1, B2, B3, B5, B6, B12 vitaminleri, C vitamini, magnezyum ve kalsiyum içerir. "Enerji serumu" olarak da bilinir. Kronik yorgunluk, migren, fibromiyalji ve üst solunum yolu enfeksiyonlarında kanıtlanmış etkinliğe sahiptir.</p>

<h2>Myers Kokteyl Temel Faydaları</h2>
<ul>
<li>🔹 <strong>Enerji artışı:</strong> Yorgunluk ve halsizliği azaltmaya yardımcı olur</li>
<li>🔹 <strong>Bağışıklık desteği:</strong> Vücudun direncini güçlendirir</li>
<li>🔹 <strong>Kas ve sinir desteği:</strong> Magnezyum ve B vitaminleri ile destek sağlar</li>
<li>🔹 <strong>Stres ve anksiyete azaltma:</strong> Rahatlamaya katkı sağlayabilir</li>
<li>🔹 <strong>Migren desteği:</strong> Migren ataklarının azalmasına yardımcı olabilir</li>
<li>🔹 <strong>Genel toparlanma:</strong> Vücudu hızlı şekilde destekler ve dinçlik sağlar</li>
</ul>

<h2>Tedavi Süreci Nasıldır?</h2>
<ul>
<li>Yavaş IV push veya kısa infüzyon şeklinde 20-30 dakikada uygulanır</li>
<li>Haftada 1 seans, gerektiğinde yoğunlaştırılabilir</li>
<li>Başlangıç kürü: 4-8 hafta, sonra idame haftada veya ayda 1</li>
</ul>

<h2>Hangi Bulgular İçin Uygundur?</h2>
<ul>
<li>Kronik yorgunluk sendromu</li>
<li>Migren ve tekrarlayan baş ağrıları</li>
<li>Fibromiyalji</li>
<li>Mevsimsel alerjiler ve astım</li>
<li>Üst solunum yolu enfeksiyonları</li>
<li>Spor sonrası toparlanma</li>
</ul>

<h2>Yan Etkileri Var Mıdır?</h2>
<p>Çok güvenli bir uygulamadır. Nadiren hissedilen sıcaklık hissi (magnezyumdan), hafif baş dönmesi görülebilir. Ciddi yan etki riski minimumdur.</p>

<h2>Hangi Tip Hastalar Kullanır?</h2>
<p>Kronik yorgunluk hastaları, migren hastaları, fibromiyalji hastaları, sporcular, yoğun stres altındaki bireyler ve genel bağışıklık desteği arayanlar Myers Kokteylinden faydalanır.</p>`,
    },
    {
        title: "NAD+ Serum Tedavisi",
        slug: "nad-plus-serum-tedavisi",
        short_description: "Hücresel enerji üretimi ve yaşlanma karşıtı tedavide NAD+ (Nikotinamid Adenin Dinükleotid) infüzyonu.",
        cover_image: "/images/nad+serum.png",
        meta_title: "Evde NAD+ Serum Tedavisi İstanbul | Anti-Aging | City in Health",
        meta_description: "NAD+ serum tedavisi İstanbul'da evinizde. Hücre yenileme, enerji artırma, anti-aging ve nöroprotektif destek. 39 ilçede 7/24.",
        content_html: `
<h2>NAD+ Serum Tedavisi Nedir?</h2>
<p>NAD+ (Nikotinamid Adenin Dinükleotid), vücuttaki her hücrede bulunan ve hücresel enerji üretiminde kritik rol oynayan bir koenzimdir. Yaşla birlikte NAD+ seviyeleri düşer. IV NAD+ infüzyonu, hücresel enerji üretimini artırır, DNA onarımını destekler, nöroplastisiteyi güçlendirir ve yaşlanma sürecini yavaşlatır.</p>

<h2>NAD+ (NAD) Temel Faydaları</h2>
<ul>
<li>🔹 <strong>Enerji üretimi:</strong> Hücrelerin enerji üretimini artırmaya yardımcı olur</li>
<li>🔹 <strong>Zihinsel performans:</strong> Odak, hafıza ve konsantrasyonu destekler</li>
<li>🔹 <strong>Yaşlanma karşıtı etki:</strong> Hücresel yenilenmeyi destekler</li>
<li>🔹 <strong>Yorgunluk azaltma:</strong> Fiziksel ve mental yorgunluğu azaltabilir</li>
<li>🔹 <strong>Metabolizma desteği:</strong> Hücre fonksiyonlarının düzenli çalışmasına katkı sağlar</li>
<li>🔹 <strong>Detoks desteği:</strong> Hücrelerin kendini temizleme süreçlerini destekler</li>
</ul>

<h2>Tedavi Süreci Nasıldır?</h2>
<ul>
<li>Yavaş IV infüzyon şeklinde 2-4 saat sürer (doza bağlı)</li>
<li>Başlangıç kürü: 3-5 gün ardışık seans</li>
<li>İdame: Ayda 1-2 seans</li>
<li>İnfüzyon hızı kişiye göre ayarlanır</li>
</ul>

<h2>Hangi Bulgular İçin Uygundur?</h2>
<ul>
<li>Yaşlanma belirtileri ve anti-aging destek</li>
<li>Kognitif fonksiyon düşüklüğü, beyin sisi</li>
<li>Kronik yorgunluk</li>
<li>Nörodejeneratif hastalıklarda destek</li>
<li>Bağımlılık tedavisinde destek</li>
<li>Atletik performans artırma</li>
</ul>

<h2>Yan Etkileri Var Mıdır?</h2>
<p>İnfüzyon sırasında göğüste sıkışma, kramp, bulantı veya baş ağrısı görülebilir. Bu etkiler infüzyon hızı yavaşlatılarak kontrol edilir. Ciddi yan etki riski düşüktür.</p>

<h2>Hangi Tip Hastalar Kullanır?</h2>
<p>Yaşlanma etkilerini yavaşlatmak isteyenler, zihinsel performansını artırmak isteyenler, kronik yorgunluk hastaları, sporcular ve nörolojik destek arayanlar NAD+ tedavisinden faydalanır.</p>`,
    },
    {
        title: "Stikolin Serum Tedavisi",
        slug: "stikolin-serum-tedavisi",
        short_description: "Beyin fonksiyonlarını destekleyen, hafıza ve konsantrasyonu artıran sitidin-5-difosfokolin (CDP-Kolin) tedavisi.",
        cover_image: "/images/stikolin.png",
        meta_title: "Evde Stikolin Serum Tedavisi İstanbul | City in Health",
        meta_description: "Stikolin (CDP-Kolin) serum tedavisi İstanbul'da evinizde. Beyin fonksiyonu, hafıza ve konsantrasyon desteği. 39 ilçede 7/24.",
        content_html: `
<h2>Stikolin Serum Tedavisi Nedir?</h2>
<p>Stikolin (Sitikolin / CDP-Kolin), beyindeki fosfolipid sentezini artıran ve nöroprotektif etkileri olan bir nootopik ajandır. IV uygulandığında beyin kan akışını artırır, nörotransmitter üretimini destekler ve nöron membranlarının onarımına katkıda bulunur. İnme, travmatik beyin hasarı ve kognitif bozukluklarda kullanılır.</p>

<h2>Tedavi Süreci Nasıldır?</h2>
<ul>
<li>IV infüzyon veya IM enjeksiyon şeklinde uygulanır</li>
<li>Uygulama süresi: 15-30 dakika</li>
<li>Genellikle günde 1 kez, 10-14 gün kür</li>
</ul>

<h2>Hangi Bulgular İçin Uygundur?</h2>
<ul>
<li>İnme sonrası rehabilitasyon</li>
<li>Travmatik beyin hasarı</li>
<li>Yaşa bağlı kognitif gerileme</li>
<li>Hafıza ve konsantrasyon problemleri</li>
<li>Nörodejeneratif hastalıklarda destek (Alzheimer, Parkinson)</li>
</ul>

<h2>Yan Etkileri Var Mıdır?</h2>
<p>Genel olarak çok güvenlidir. Nadir: baş ağrısı, uykusuzluk, mide bulantısı, ishal. Ciddi yan etki riski çok düşüktür.</p>

<h2>Hangi Tip Hastalar Kullanır?</h2>
<p>İnme geçiren hastalar, beyin travması sonrası rehabilitasyon görenler, yaşlılarda kognitif destek arayanlar ve zihinsel performansını artırmak isteyenler bu tedaviden faydalanır.</p>`,
    },
    {
        title: "Koenzim Q10 Serum Tedavisi",
        slug: "koenzim-q10-serum-tedavisi",
        short_description: "Hücresel enerji üretimi, kalp sağlığı ve antioksidan koruma için CoQ10 intravenöz tedavisi.",
        cover_image: "/images/koenzimQ10.png",
        meta_title: "Evde Koenzim Q10 Serum Tedavisi İstanbul | City in Health",
        meta_description: "Koenzim Q10 serum tedavisi İstanbul'da evinizde. Kalp sağlığı, enerji üretimi ve antioksidan destek. 39 ilçede 7/24.",
        content_html: `
<h2>Koenzim Q10 Serum Tedavisi Nedir?</h2>
<p>Koenzim Q10 (Ubikinon), mitokondride enerji üretiminde kritik rol oynayan bir koenzimdir. Özellikle kalp, karaciğer ve böbrek gibi yüksek enerji gerektiren organlarda yoğun bulunur. IV yoluyla uygulandığında oral formlardan çok daha yüksek biyoyararlanım sağlar. Kalp yetmezliği, statin kullanan hastalar ve kronik yorgunlukta etkilidir.</p>

<h2>Tedavi Süreci Nasıldır?</h2>
<ul>
<li>IV infüzyon 20-40 dakikada uygulanır</li>
<li>Haftada 1-2 seans, 8-12 seans kür</li>
<li>Statin kullanan hastalarda sürekli destek önerilebilir</li>
</ul>

<h2>Hangi Bulgular İçin Uygundur?</h2>
<ul>
<li>Kalp yetmezliği ve kardiyomiyopati</li>
<li>Statin ilacı kullanımına bağlı kas ağrıları</li>
<li>Kronik yorgunluk sendromu</li>
<li>Migren profilaksisi</li>
<li>Yaşlanma karşıtı hücresel destek</li>
</ul>

<h2>Yan Etkileri Var Mıdır?</h2>
<p>Genellikle iyi tolere edilir. Çok nadir: mide bulantısı, ishal, uykusuzluk. Kan sulandırıcı kullanan hastalarda dikkatli olunmalıdır.</p>

<h2>Hangi Tip Hastalar Kullanır?</h2>
<p>Kalp hastaları, statin kullanan bireyler, kronik yorgunluk hastaları, migren hastaları ve anti-aging destek arayanlar CoQ10 serumundan faydalanır.</p>`,
    },
    {
        title: "B-Complex Serum Tedavisi",
        slug: "b-complex-serum-tedavisi",
        short_description: "Tüm B grubu vitaminlerini bir arada içeren kapsamlı vitamin B kompleksi IV tedavisi.",
        cover_image: "/images/B12-vitamin.png",
        meta_title: "Evde B-Complex Serum Tedavisi İstanbul | City in Health",
        meta_description: "B-Complex serum tedavisi İstanbul'da evinizde. Enerji, sinir sistemi ve metabolizma desteği. 39 ilçede 7/24.",
        content_html: `
<h2>B-Complex Serum Tedavisi Nedir?</h2>
<p>B-Complex serum tedavisi, B1, B2, B3, B5, B6 ve B12 vitaminlerinin bir arada intravenöz yolla uygulanmasıdır. B grubu vitaminler enerji metabolizması, sinir sistemi fonksiyonları, kırmızı kan hücresi üretimi ve hormon sentezinde kritik rol oynar. IV uygulama ile emilim sorunları aşılır ve hızlı etki sağlanır.</p>

<h2>Tedavi Süreci Nasıldır?</h2>
<ul>
<li>IV infüzyon veya IM enjeksiyon şeklinde uygulanır</li>
<li>Uygulama: 20-30 dakika</li>
<li>Haftada 1-2 seans, 4-8 hafta kür</li>
</ul>

<h2>Hangi Bulgular İçin Uygundur?</h2>
<ul>
<li>B vitamini eksikliği</li>
<li>Kronik yorgunluk ve enerji düşüklüğü</li>
<li>Nöropati ve sinir sistemi sorunları</li>
<li>Anemi (B12/folat eksikliği)</li>
<li>Stres ve anksiyete desteği</li>
<li>Saç, cilt ve tırnak sağlığı</li>
</ul>

<h2>Yan Etkileri Var Mıdır?</h2>
<p>Çok güvenlidir. B vitaminkleri suda çözünür, fazlası idrarla atılır. Nadir: uygulama yerinde hafif ağrı, idrar renginde koyulaşma (normal).</p>

<h2>Hangi Tip Hastalar Kullanır?</h2>
<p>Vitamin B eksikliği olanlar, vejetaryen/veganlar, yaşlılar, mide-barsak emilim bozukluğu olanlar, alkolik bireyler ve kronik yorgunluk yaşayanlar bu tedaviden faydalanır.</p>`,
    },
];

// B Vitamins with dedicated photos
export const B_VITAMINS_WITH_PHOTOS = [
    {
        title: "B1 Vitamin (Thiamine) Serum Tedavisi",
        slug: "b1-vitamin-thiamine-serum-tedavisi",
        short_description: "Enerji metabolizması ve sinir sistemi için B1 vitamini (Tiamin) IV uygulaması.",
        cover_image: "/images/b1.png",
        meta_title: "Evde B1 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "B1 Vitamin (Thiamine) serum tedavisi İstanbul'da evinizde. Enerji metabolizması ve sinir sistemi desteği. 39 ilçede 7/24.",
        content_html: `<h2>B1 Vitamin (Thiamine) Serum Tedavisi Nedir?</h2><p>Tiamin (B1), karbonhidrat metabolizmasında ve sinir sistemi fonksiyonlarında anahtar rol oynar. IV tiamin tedavisi özellikle Wernicke ensefalopatisi, alkolizm, malnütrisyon ve nöropatide kullanılır.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Wernicke ensefalopatisi</li><li>Alkolizme bağlı vitamin eksikliği</li><li>Periferik nöropati</li><li>Kronik yorgunluk</li><li>Kalp yetmezliği desteği</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Genellikle güvenlidir. Çok nadir alerjik reaksiyon görülebilir. IV uygulama yavaş yapılmalıdır.</p>`,
    },
    {
        title: "B2 Vitamin (Riboflavin) Serum Tedavisi",
        slug: "b2-vitamin-riboflavin-serum-tedavisi",
        short_description: "Enerji üretimi, cilt sağlığı ve migren profilaksisinde B2 vitamini IV uygulaması.",
        cover_image: "/images/b2.png",
        meta_title: "Evde B2 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "B2 Vitamin (Riboflavin) serum tedavisi İstanbul'da evinizde. Migren profilaksisi ve enerji desteği. 39 ilçede 7/24.",
        content_html: `<h2>B2 Vitamin (Riboflavin) Serum Tedavisi Nedir?</h2><p>Riboflavin (B2), FAD ve FMN koenzimlerinin yapı taşıdır. Enerji üretimi, hücresel büyüme, yağ metabolizması ve antioksidan savunmada rol oynar. Yüksek doz riboflavin migren profilaksisinde kanıtlanmış etkinliğe sahiptir.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Migren profilaksisi (400mg/gün)</li><li>B2 vitamini eksikliği</li><li>Ağız yarası ve dudak çatlakları</li><li>Anemi desteği</li><li>Göz sağlığı</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Çok güvenli bir vitamindir. İdrarı parlak sarıya boyar, bu normal bir durumdur. Yan etki riski çok düşüktür.</p>`,
    },
    {
        title: "B12 Vitamin (Metilkobalamin) Serum Tedavisi",
        slug: "b12-vitamin-metilkobalamin-serum-tedavisi",
        short_description: "Kansızlık, nöropati ve enerji eksikliğinde metilkobalamin (aktif B12) IV/IM tedavisi.",
        cover_image: "/images/B12-vitamin.png",
        meta_title: "Evde B12 Vitamin Serum Tedavisi İstanbul | Metilkobalamin | City in Health",
        meta_description: "B12 Vitamin (Metilkobalamin) serum tedavisi İstanbul'da evinizde. Anemi, nöropati ve enerji desteği. 39 ilçede 7/24.",
        content_html: `<h2>B12 Vitamin (Metilkobalamin) Serum Tedavisi Nedir?</h2><p>Metilkobalamin, B12 vitamininin en aktif ve biyolojik olarak kullanılabilir formudur. Kırmızı kan hücresi üretimi, DNA sentezi, sinir sistemi fonksiyonları ve enerji metabolizmasında hayati öneme sahiptir. Mide-barsak emilim sorunlarında IV/IM uygulama en etkili yöntemdir.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Megaloblastik (pernisiyöz) anemi</li><li>Periferik nöropati, uyuşma, karıncalanma</li><li>Kronik yorgunluk ve halsizlik</li><li>Vejetaryen/vegan bireylerde eksiklik</li><li>Mide ameliyatı sonrası emilim bozukluğu</li><li>Yaşlılarda kognitif destek</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Çok güvenlidir. Suda çözünür, fazlası idrarla atılır. IM enjeksiyonda uygulama yerinde hafif ağrı olabilir. Ciddi yan etki riski yoktur.</p>`,
    },
];

// Mineral and other services
export const MINERAL_SERVICES = [
    {
        title: "Çinko Serum Tedavisi",
        slug: "cinko-serum-tedavisi",
        short_description: "Bağışıklık güçlendirme, yara iyileşmesi ve cilt sağlığında çinko IV tedavisi.",
        cover_image: "/images/cinko.png",
        meta_title: "Evde Çinko Serum Tedavisi İstanbul | City in Health",
        meta_description: "Çinko serum tedavisi İstanbul'da evinizde. Bağışıklık, yara iyileşmesi ve cilt sağlığı desteği. 39 ilçede 7/24.",
        content_html: `<h2>Çinko Serum Tedavisi Nedir?</h2><p>Çinko, 300'den fazla enzim reaksiyonunda görev alan esansiyel bir mineraldir. Bağışıklık sistemi, yara iyileşmesi, DNA sentezi, protein üretimi ve hücre bölünmesinde kritik öneme sahiptir. IV çinko tedavisi eksikliğin hızla düzeltilmesini sağlar.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Çinko eksikliği</li><li>Bağışıklık sistemi zayıflığı</li><li>Yara iyileşmesinde gecikme</li><li>Saç dökülmesi ve cilt sorunları</li><li>Tat ve koku kaybı</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Normal dozlarda güvenlidir. Yüksek dozda bulantı, kusma, bakır eksikliği görülebilir. Bakır takibi önerilir.</p>`,
    },
    {
        title: "Selenyum Serum Tedavisi",
        slug: "selenyum-serum-tedavisi",
        short_description: "Tiroid fonksiyonu, antioksidan savunma ve kanser korumasında selenyum IV tedavisi.",
        cover_image: "/images/selenyum.png",
        meta_title: "Evde Selenyum Serum Tedavisi İstanbul | City in Health",
        meta_description: "Selenyum serum tedavisi İstanbul'da evinizde. Tiroid, antioksidan ve bağışıklık desteği. 39 ilçede 7/24.",
        content_html: `<h2>Selenyum Serum Tedavisi Nedir?</h2><p>Selenyum, glutatyon peroksidaz ve tiyoredoksin redüktaz gibi antioksidan enzimlerin yapısında bulunan esansiyel bir eser elementtir. Tiroid fonksiyonu, bağışıklık sistemi ve antioksidan savunmada kritik rol oynar.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Selenyum eksikliği</li><li>Tiroid hastalıkları (Hashimoto)</li><li>Antioksidan destek</li><li>Bağışıklık güçlendirme</li><li>Kardiyovasküler koruma</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Normal dozlarda güvenlidir. Aşırı dozda selenozis (sarımsak kokulu nefes, saç dökülmesi, tırnak kırılması) görülebilir. Doz takibi önemlidir.</p>`,
    },
    {
        title: "Biotin Serum Tedavisi",
        slug: "biotin-serum-tedavisi",
        short_description: "Saç, cilt ve tırnak sağlığı ile enerji metabolizmasında biotin (B7) IV tedavisi.",
        cover_image: "/images/biotin.png",
        meta_title: "Evde Biotin Serum Tedavisi İstanbul | City in Health",
        meta_description: "Biotin serum tedavisi İstanbul'da evinizde. Saç dökülmesi, tırnak kırılması ve cilt sağlığı desteği. 39 ilçede 7/24.",
        content_html: `<h2>Biotin Serum Tedavisi Nedir?</h2><p>Biotin (B7 / H vitamini), keratin üretimini destekleyen ve saç, cilt, tırnak sağlığında önemli rol oynayan bir B vitaminidir. Ayrıca yağ, karbonhidrat ve protein metabolizmasında kofaktör olarak görev yapar.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Saç dökülmesi ve incelmesi</li><li>Tırnak kırılması</li><li>Cilt kuruluğu ve dermatit</li><li>Biotin eksikliği</li><li>Diyabet yönetiminde destek</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Çok güvenlidir. Bilinen ciddi yan etkisi yoktur. Yüksek doz biotin bazı laboratuvar testlerini etkileyebilir, tahlil öncesi doktorunuzu bilgilendirin.</p>`,
    },
    {
        title: "Folik Asit Serum Tedavisi",
        slug: "folik-asit-serum-tedavisi",
        short_description: "Hamilelik desteği, anemi tedavisi ve hücre yenilenmesinde folik asit (B9) IV tedavisi.",
        cover_image: "/images/Folikasit.png",
        meta_title: "Evde Folik Asit Serum Tedavisi İstanbul | City in Health",
        meta_description: "Folik asit serum tedavisi İstanbul'da evinizde. Hamilelik, anemi ve hücre yenileme desteği. 39 ilçede 7/24.",
        content_html: `<h2>Folik Asit (B9) Serum Tedavisi Nedir?</h2><p>Folik asit (B9), DNA sentezi, hücre bölünmesi ve kırmızı kan hücresi üretiminde esansiyel bir vitamindir. Hamilelikte nöral tüp defektlerinin önlenmesinde hayati öneme sahiptir. IV uygulama ciddi eksikliklerde ve emilim bozukluklarında tercih edilir.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Megaloblastik anemi</li><li>Hamilelikte folik asit desteği</li><li>Homosistein yüksekliği</li><li>Malabsorpsiyon sendromu</li><li>Metotreksat tedavisi alan hastalar</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Çok güvenlidir. Suda çözünür, fazlası idrarla atılır. Nadir: alerjik reaksiyon. B12 eksikliğini maskeleyebilir, B12 düzeyi kontrol edilmelidir.</p>`,
    },
];

// B Vitamins without dedicated photos (listed by number order at the end)
export const B_VITAMINS_OTHER = [
    {
        title: "B3 Vitamin (Niyasin) Serum Tedavisi",
        slug: "b3-vitamin-niyasin-serum-tedavisi",
        short_description: "Kolesterol düzenleme, enerji metabolizması ve cilt sağlığında B3 vitamini IV tedavisi.",
        cover_image: "/images/evde_serum_genel.png",
        meta_title: "Evde B3 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "B3 Vitamin (Niyasin) serum tedavisi İstanbul'da evinizde. Kolesterol düzenleme ve enerji desteği. 39 ilçede 7/24.",
        content_html: `<h2>B3 Vitamin (Niyasin) Serum Tedavisi Nedir?</h2><p>Niyasin (B3 / nikotinik asit), NAD ve NADP koenzimlerinin öncüsüdür. 500'den fazla enzim reaksiyonunda görev alır. Kolesterol profili düzenleme, enerji metabolizması ve DNA onarımında kritik rol oynar.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Yüksek kolesterol (trigliserid düşürme)</li><li>Pellegra hastalığı</li><li>Enerji metabolizması bozukluğu</li><li>Cilt hastalıkları</li><li>Kardiyovasküler risk azaltma</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>En bilinen yan etkisi "flushing" (yüzde kızarma, sıcaklık hissi) dir. Bu geçici ve zararsızdır. Yüksek dozlarda karaciğer enzim takibi önerilir.</p>`,
    },
    {
        title: "B4 Vitamin (Choline Chloride) Serum Tedavisi",
        slug: "b4-vitamin-choline-chloride-serum-tedavisi",
        short_description: "Karaciğer sağlığı, beyin fonksiyonu ve yağ metabolizmasında kolin klorür IV tedavisi.",
        cover_image: "/images/evde_serum_genel.png",
        meta_title: "Evde B4 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "Kolin Klorür (B4) serum tedavisi İstanbul'da evinizde. Karaciğer ve beyin sağlığı desteği. 39 ilçede 7/24.",
        content_html: `<h2>B4 Vitamin (Kolin Klorür) Serum Tedavisi Nedir?</h2><p>Kolin, hücre membranı yapısı, asetilkolin nörotransmitter üretimi ve yağ metabolizmasında esansiyel bir besindir. IV kolin klorür uygulaması karaciğer yağlanmasını azaltır, beyin fonksiyonlarını destekler.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Karaciğer yağlanması</li><li>Hafıza ve kognitif fonksiyon desteği</li><li>Yağ metabolizması bozuklukları</li><li>Nörolojik destek</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Genellikle iyi tolere edilir. Yüksek dozlarda balıksı vücut kokusu, mide bulantısı görülebilir.</p>`,
    },
    {
        title: "B5 Vitamin (Pantotenik Asit) Serum Tedavisi",
        slug: "b5-vitamin-pantotenik-asit-serum-tedavisi",
        short_description: "Hormon sentezi, yara iyileşmesi ve stres yönetiminde pantotenik asit IV tedavisi.",
        cover_image: "/images/evde_serum_genel.png",
        meta_title: "Evde B5 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "B5 Vitamin (Pantotenik Asit) serum tedavisi İstanbul'da evinizde. Stres yönetimi ve hormon desteği. 39 ilçede 7/24.",
        content_html: `<h2>B5 Vitamin (Pantotenik Asit) Serum Tedavisi Nedir?</h2><p>Pantotenik asit (B5), Koenzim A sentezinin temel bileşenidir. Yağ asidi metabolizması, hormon sentezi (kortizol, melatonin), yara iyileşmesi ve hücresel enerji üretiminde rol oynar. "Anti-stres vitamini" olarak da bilinir.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Kronik stres ve adrenal yorgunluk</li><li>Yara iyileşmesi desteği</li><li>Akne ve cilt sorunları</li><li>Romatoid artrit desteği</li><li>Kolesterol metabolizması</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Çok güvenlidir. Suda çözünür, fazlası vücuttan atılır. Yan etki riski çok düşüktür.</p>`,
    },
    {
        title: "B6 Vitamin (Piridoksin) Serum Tedavisi",
        slug: "b6-vitamin-piridoksin-serum-tedavisi",
        short_description: "Sinir sistemi, bağışıklık ve hemoglobin üretiminde B6 vitamini IV tedavisi.",
        cover_image: "/images/evde_serum_genel.png",
        meta_title: "Evde B6 Vitamin Serum Tedavisi İstanbul | City in Health",
        meta_description: "B6 Vitamin (Piridoksin) serum tedavisi İstanbul'da evinizde. Sinir sistemi ve bağışıklık desteği. 39 ilçede 7/24.",
        content_html: `<h2>B6 Vitamin (Piridoksin) Serum Tedavisi Nedir?</h2><p>Piridoksin (B6), 100'den fazla enzim reaksiyonunda kofaktör olarak görev yapar. Amino asit metabolizması, hemoglobin sentezi, nörotransmitter üretimi (serotonin, dopamin, GABA) ve bağışıklık fonksiyonlarında kritik rol oynar.</p><h2>Hangi Bulgular İçin Uygundur?</h2><ul><li>Hamilelikte bulantı (hiperemezis gravidarum)</li><li>Premenstrüel sendrom (PMS)</li><li>Karpal tünel sendromu</li><li>Nöropati</li><li>Anemi</li><li>Homosistein yüksekliği</li></ul><h2>Yan Etkileri Var Mıdır?</h2><p>Normal dozlarda güvenlidir. Uzun süreli çok yüksek dozlarda periferik nöropati riski vardır. Doktor takibinde uygulanmalıdır.</p>`,
    },
];
